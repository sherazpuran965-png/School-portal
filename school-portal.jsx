...
const STRINGS = {
  en: {
    ...
    tabs: { admin: "Administration", teachers: "Teachers", students: "Students", fees: "Fees" },
  ...
  ur: {
    ...
    tabs: { admin: "انتظامیہ", teachers: "اساتذہ", students: "طلبہ", fees: "فیس" },
  ...
  ar: {
    ...
    tabs: { admin: "الإدارة", teachers: "المعلمون", students: "الطلاب", fees: "الرسوم" },
    ...
    <Card onClick={onClick} style={{ marginBottom: 10, cursor: "pointer" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {collected !== undefined && (
            <div style={{ fontSize: 12, color: COLORS.sub, marginTop: 2 }}>
              {t.collected} ${collected.toLocaleString()} &middot; {t.outstanding} ${outstanding.toLocaleString()}
            </div>
          )}
        </div>
...
function FeeLineItem({ label, item, onSave, onDelete, editableLabel, t }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: `1px solid ${COLORS.border}` }} onClick={(e) => e.stopPropagation()}>
      ...
      <div style={{ fontSize: 13, fontWeight: 600, whiteSpace: "nowrap" }}>${item.amount.toLocaleString()}</div>
      ...
      <div onClick={() => setOpen((o) => !o)} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: outstanding > 0 ? COLORS.rose : COLORS.sage, fontWeight: 600, marginTop: 2 }}>
            {t.totalOutstanding}: ${outstanding.toLocaleString()}
          </div>
        </div>
...
// ---------- teacher salary card (amount, payment method, pay action) ----------
function TeacherSalaryCard({ teacher, onSave, t }) {
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState(teacher.salary);

  function openEdit(e) {
    e.stopPropagation();
    setDraft(teacher.salary);
    setEditing(true);
    setOpen(true);
  }
  function save(e) {
    e.stopPropagation();
    onSave({ ...teacher, salary: { ...draft, amount: Number(draft.amount) || 0 } });
    setEditing(false);
  }
  function cancelEdit(e) {
    e.stopPropagation();
  ...
  function quickPay(e) {
    e.stopPropagation();
    onSave({ ...teacher, salary: { ...teacher.salary, status: "paid", paidDate: new Date().toISOString().slice(0, 10) } });
  }
    ...
    <Card style={{ marginBottom: 10 }}>
      <div onClick={() => setOpen((o) => !o)} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {!open && (
            <div style={{ fontSize: 12, color: COLORS.sub, marginTop: 2 }}>
              ${teacher.salary.amount.toLocaleString()} &middot; {teacher.salary.method === "bank" ? t.methodBank : t.methodCash}
            </div>
          )}
        </div>
        <StatusBadge status={teacher.salary.status === "paid" ? "paid" : "pending"} t={t} />
        {open ? <ChevronUp size={16} color={COLORS.sub} /> : <ChevronDown size={16} color={COLORS.sub} />}
      </div>
          ...
          {editing ? (
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {["bank", "cash"].map((m) => (
                    ...
                    >
                      {m === "bank" ? t.methodBank : t.methodCash}
                    </button>
                  ))}
                </div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: COLORS.sub, marginBottom: 3 }}>{t.statusLabel}</div>
                <div style={{ display: "flex", gap: 6 }}>
                  {["paid", "pending"].map((st) => (
                    <button
                      key={st}
                      onClick={() =>
                        setDraft((d) => ({
                          ...d,
                          status: st,
                          paidDate: st === "paid" ? d.paidDate || new Date().toISOString().slice(0, 10) : null,
                        }))
                      }
                      style={{
                        flex: 1,
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                <span style={{ color: COLORS.sub }}>{t.salaryAmountLabel}</span>
                <span style={{ fontWeight: 600 }}>${teacher.salary.amount.toLocaleString()}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                <span style={{ color: COLORS.sub }}>{t.paymentMethodLabel}</span>
                <span style={{ fontWeight: 600 }}>{teacher.salary.method === "bank" ? t.methodBank : t.methodCash}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, alignItems: "center" }}>
                <span style={{ color: COLORS.sub }}>{t.statusLabel}</span>
                <StatusBadge status={teacher.salary.status === "paid" ? "paid" : "pending"} t={t} />
              </div>
              {teacher.salary.status === "paid" && teacher.salary.paidDate && (
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                  <span style={{ color: COLORS.sub }}>{t.paidOn}</span>
                  <span style={{ fontWeight: 600 }}>{teacher.salary.paidDate}</span>
                </div>
              )}
...
export default function SchoolPortal() {
  ...
  const [schoolName, setSchoolName] = useState({
    en: "Greenfield Academy",
    ur: "گرین فیلڈ اکیڈمی",
    ar: "أكاديمية جرين فيلد",
  });
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: COLORS.sub }}>
                        {getClassName(s.classId)} &middot; ${studentOutstanding(s).toLocaleString()}
                      </div>
                    </div>
              ...
              <>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 18 }}>
                  <Card>
                    ...
                    <div style={{ fontFamily: "Georgia, serif", fontSize: 22, fontWeight: 700, color: COLORS.sage }}>${feeStats.collected.toLocaleString()}</div>
                  </Card>
                  <Card>
                    <div style={{ fontSize: 12, color: COLORS.sub, marginBottom: 6 }}>{t.outstanding}</div>
                    <div style={{ fontFamily: "Georgia, serif", fontSize: 22, fontWeight: 700, color: COLORS.rose }}>${feeStats.outstanding.toLocaleString()}</div>
                  </Card>
                </div>
